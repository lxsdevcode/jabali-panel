...
#   4. Clones (or pulls) https://git.jabali-panel.com/shukivaknin/jabali2
...
#
# Usage (public repo):
#   curl -fsSL https://git.jabali-panel.com/shukivaknin/jabali2/raw/branch/main/install.sh | bash
#
...
REPO_URL="${JABALI_REPO_URL:-https://git.jabali-panel.com/shukivaknin/jabali2.git}"
...
clone_or_update_repo() {
  ...
  local attempt
  for attempt in 1 2 3 4 5 6 7 8; do
    if getent hosts "${REPO_HOST:-git.jabali-panel.com}" >/dev/null 2>&1; then
      _clone_dns_ok=1
      break
    fi
    sleep 2
