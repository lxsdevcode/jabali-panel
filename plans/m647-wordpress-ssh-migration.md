...
**Tasks**
- New agent verb `migration.import_wp`: input `{staging dir (holding `files.tar.gz` + `dump.sql`), dest user/domain/docroot, domain_change, old_url, new_url, prefix}`. Create the Jabali DB + user; import `dump.sql`; **safe-extract** `files.tar.gz` → dest docroot (**containment**: write ONLY under docroot, reuse `TestMigrationImportHomeContainment` logic, reject symlink/`..`/zip-slip — the tar came from an untrusted source).
