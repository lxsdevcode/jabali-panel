...
- **NEXT: S4 — the shared `import_wp` spine** (destructive half; fresh session). Now carries operator guidance: **reuse `cpanel/restore_appconfigs.rewriteWordPress`** for the wp-config DB-cred rewrite (like the cPanel migration) + **Duplicator-style serialized-safe `wp search-replace`** for domain-change. Containment on the untrusted `files.tar.gz` extraction is the S4/S9 gate.
